...
if __name__ == "__main__":
    ...
    my_AE = AutoEncoder(config)    
    my_AE.do_init(config.DBN_init)

    last_Loss = 0
    while (True):
        mini_batch = graph_data.sample(config.batch_size)
        st_time = time.time()
        my_AE.fit(mini_batch)
        ...
        if graph_data.is_epoch_end():
            ...
            while (True):
                mini_batch = graph_data.sample(config.batch_size)
                Loss += my_AE.get_Loss(mini_batch)
                if graph_data.is_Epoch_end():
                    break
            }
    ...
    ##embedding = my_AE.getEmbedding(graph_data.get_all())
